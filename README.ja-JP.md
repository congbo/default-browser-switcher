...
<p align="center">
  <img src="docs/images/menu-bar-runtime.png" alt="メニューバーの実行時スクリーンショット" width="360" />
</p>
...
### macOS で「アプリが壊れているため開けません」と表示される場合は？

macOS のセキュリティ保護により、App Store 以外から入手したアプリでこの警告が表示されることがあります。次のいずれかの方法で対処できます。
...
1. `ターミナルで修復`（推奨）
   ...
   > 注意: アプリ名を変更している場合は、コマンド内のパスを実際のアプリ名と場所に合わせて読み替えてください。

2. または、`システム設定` -> `プライバシーとセキュリティ` で `このまま開く` をクリックしてください。
